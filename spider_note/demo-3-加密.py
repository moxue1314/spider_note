...
'''
    ...
    '中' 这个汉字 ， utf-8是：  b'\xe4\xb8\xad'
    转为 二进制：  1110 01 00 1011 1000 10 10 1101
    按6个位进行分割4个字节：   111001  001011 100010 101101
    在前面补0 :    00111001  00001011  00100010  00101101

    4个字节:  b'\xe4\xb8\xad\x48'
    转为 二进制:      1110 01 00 1011 1000 10 10 1101  0100 1000
    按6个位进行分割4个字节： 111001  001011 100010 101101   010010  00
    前面4个字节处理掉，剩余尾部： 010010  00
    第二个会在前面补0，依然只有2个字节: 010010  00000000  
    转换为这个字符串的时候，假设前面2个是A0，会自动在尾部不组4个字符，即补2个 '=='： A0==
'''

# import base64
# s = '测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中测试中'
#
# #  会按57个字节的长度为间隔 加入 \n
...
'''
    ...
    2、selenium 进行浏览器模拟
    3、pyexec，下载这个JS，用pyexec调用这个js的方法
